...
public:
    ...
    {
        std::cerr << "[Parse Error] Error " << msg << " on line " << peek(-1).value().line << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    {
        if (try_consume(TokenType::warnagar)) {
        ...
        if (try_consume(TokenType::warna)) {
    ...
    {
        if (peek().has_value() && peek().value().type == TokenType::produce && peek(1).has_value()
        ...
        if (peek().has_value() && peek().value().type == TokenType::have && peek(1).has_value()
        ...
        if (auto if_ = try_consume(TokenType::agar)) {
            try_consume_err(TokenType::open_paren);
