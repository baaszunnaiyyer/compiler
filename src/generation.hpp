...
public:
    ...
    void gene_expr(const NodeExpr& expr) {
        struct ExprVisitor {
            Generator* gen;
        ...
        struct StmtVisitor {
            ...
            void operator()(const NodeStmtLet& stmt_let) const {
                if (gen->m_vars.contains(stmt_let.ident.value.value())) {
                    std::cerr << "Identifer already exist" << stmt_let.ident.value.value() << std::endl;
                    exit(EXIT_FAILURE);
                }

                gen->m_vars.insert({stmt_let.ident.value.value(), Var {.stack_loc = gen->m_stack_size}});
                gen->gene_expr(stmt_let.expr);
            }
        };
